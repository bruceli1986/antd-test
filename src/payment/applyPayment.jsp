...
<html>
	<head>
		<title><spring:message code="payment_l_applyPayment"/></title>
	...
	<body>
		<div class="payment-path">
			<ol class="breadcrumb ">
				<li class="active"><spring:message code="payment_l_applyPayment"/></li>
			</ol>
		</div>
		
		<div class="row">
			<div class="col-md-12" role="main">
				<form id="paymentForm" class="form-horizontal" action="<c:url value="/payment2/applyPayment/add"/>" method="post">
					...
					<div class="form-group">
						...
						<div class="col-md-4">
							<p class="form-control-static">${po.project}</p>
						</div>
						<label class="control-label col-md-2"><spring:message code="payment_l_projectDescription"/></label>
						<div class="col-md-4">
							<p class="form-control-static">${po.projectDesc}</p>	
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-md-2"><spring:message code="payment_l_poNo"/></label>
						<div class="col-md-4">
							<p class="form-control-static"><a target="_blank" href="<c:url value="/payment2/contractDetails?poNo=${po.poNo}&id=${system.id}&code=${system.code}"/>">${po.poNo}</a></p>	
					...
					<div class="form-group">
						<label class="control-label col-md-2"><spring:message code="payment_l_vendor"/></label>
						<div class="col-md-4">
							<select class="selectpicker" name="company" >
								<c:forEach var="vendor" items="${vendors}">
									<option value="${vendor.company}" data-subtext="${vendor.company}" <c:if test="${po.company eq vendor.company}">selected</c:if>>${vendor.description}</option>
								</c:forEach>
							</select>
						</div>
						<label class="control-label col-md-2"><spring:message code="payment_l_invoiceNo"/></label>
						<div class="col-md-4">
							<input class="form-control" type="text" id="paymentFormNextInvoiceNo" name="invoiceNo" value="${po.nextInvoiceNo}">
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-md-4">
							...
							<input type="hidden" name="dateRcvd">
						</div>
						<label class="control-label col-md-2"><spring:message code="payment_l_apRef"/></label>
						<div class="col-md-4">
							<textarea class="form-control" rows="2" name="apRef" ></textarea>
						</div>
					</div> 	
					...
					<table id="paymentListTable" data-form="false" data-toggle="table" data-url="<c:url value="/payment2/applyPayment/queryPcList?systemCode=${system.code}&poNo=${po.poNo}"/>" data-query-params="generateQueryParams" data-classes="table table-hover table-no-bordered" data-toolbar="#paymentTableToolbar">
						<thead>
							<tr>
					...
					<p>
						...
						<div class="table-responsive">
							<table id="uploaded-files" class="table uploaded-files" data-key="[]" style="display:none;">
						        <tr>
		...
		<script>
		 $(function(){
		 	...
		 	function generateQueryParams(params){
				params.company=$('select[name=company]').val();
				return params;
			}
			...
			function filterFormTr(){
		 	...
		 	function init(){
		 		...
		 		$('#paymentListTable').on('load-success.bs.table', function(event, data){	
		 			if(data.length){
		 		...
		 		$('select[name=company]').on('change', function (){
					$('#paymentListTable').bootstrapTable('refresh');
					$('select[name=company]').val($('select[name=company] option:selected').val());
				});
				...
				$('#payment-form-type').bootstrapSwitch({
			 		...
			 		offColor:'primary'
				  	...
				  	if(state == false){
				  		...
				  		filterFormTr();
				  		$('#paymentListTable').bootstrapTable('resetView');	
				  	}else{
				...
				$('#datetimepicker').datepicker('setDate', new Date().toDateString());
		 	}
				...
				$('#paymentForm').validate({
					rules:{
						...
						invoiceNo:{
							required: true ,
							remote:{
								url:"<c:url value="/payment2/applyPayment/checkInvoiceNo"/>",
								data:{
									poNo: function() {
										return "${po.poNo}";
									},
					...
					messages:{
						...
						invoiceNo:{
							required:'<spring:message code="payment_m_error8"/>',
							remote:'<spring:message code="payment_m_error9"/>'
						}
					},
					...
					submitHandler: function (form){
						...
						$.ajax({
			                ...
			                success: function(data) {
			                   ...
			                   if(data == true){
		                    		swal({   
		                    			title: '<spring:message code="payment_m_info1"/>',   
